  ...
  return (
    <>
      ...
      <nav className={`w-full z-50 transition-all duration-300 ${
        isScrolled ? 'bg-[#1c1c1c]/90 backdrop-blur-lg shadow-md border-b border-[#e5c79a40]' : 'bg-[#1c1c1c]'
      }`}>
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          {/* Desktop Links */}
          <ul className="hidden md:flex space-x-8 text-[#e5c79a] font-medium tracking-wide">
            {['Home', 'Shop', 'Collections', 'About', 'Contact'].map((item, index) => (
              <li key={index}>
                <Link
  ...
  <span className="absolute -top-1 -right-1 text-[10px] bg-white text-black w-5 h-5 flex items-center justify-center rounded-full font-bold">
    {cartCount}
        ...
        <div className={`md:hidden transition-all duration-300 overflow-hidden ${
          ...
          <div className="px-6 py-4 bg-[#1c1c1c] border-t border-[#e5c79a33]">
            <div className="flex flex-col space-y-4 text-[#e5c79a]">
              {['Home', 'Shop', 'Collections', 'About', 'Contact'].map((item, index) => (
                <Link
                  key={index}
                  to={`/${item === 'Home' ? '' : item.toLowerCase()}`}
              ...
              <div className="pt-4 border-t border-[#e5c79a40]">
                <div className="relative">
                  ...
                  />
                  <FaSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#e5c79a]" size={14} />
                </div>
              </div>
            </div>
          </div>
